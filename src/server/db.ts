...
interface Channel extends cassandra.types.Row {
	uuid: string,
	name: string,
	members: string[],
	admins: string[],
...
interface Message extends cassandra.types.Row {
	uuid: string,
	channel: string,
	message: string,
	username: string
...
const DB = {
	 ...
	 */
	async addMessage (channel: string, user: string, message: string): Promise<void> {
	 ...
	 */
	async deleteMessage (channel: string, uuid: string): Promise<void> {
		if (this.closed) throw new Error('DB closed')
	 ...
	 */
	async editMessage (channel: string, uuid: string, newmessage: string): Promise<void> {
		if (this.closed) throw new Error('DB closed')
	 ...
	 */
	async getChannels (user: string): Promise<string[]> {
		if (this.closed) throw new Error('DB closed')
	 ...
	 */
	async getChannelProperties (channel: string): Promise<Channel> {
		if (this.closed) throw new Error('DB closed')
	 ...
	 */
	async createChannel (user: string, name: string): Promise<string> {
		if (this.closed) throw new Error('DB closed')
	 ...
	 */
	async isMember (user: string, channel: string): Promise<boolean> {
		if (this.closed) throw new Error('DB closed')
	 ...
	 */
	async isAdmin (user: string, channel: string): Promise<boolean> {
		if (this.closed) throw new Error('DB closed')
	 ...
	 */
	async makeAdmin (user: string, channel: string): Promise<void> {
		if (this.closed) throw new Error('DB closed')
	 ...
	 */
	async removeAdmin (user: string, channel: string): Promise<void> {
		if (this.closed) throw new Error('DB closed')
	 ...
	 * @param channel The channel to remove from
	 */
	async leaveChannel (user: string, channel: string): Promise<void> {
		if (this.closed) throw new Error('DB closed')
	 ...
	 * @returns The message
	 */
	async getSingleMessage (channel: string, uuid: string): Promise<Message> {
		if (this.closed) throw new Error('DB closed')
	 ...
	 */
	async getMessageSender (channel: string, uuid: string): Promise<string> {
		if (this.closed) throw new Error('DB closed')
	 ...
	 * @deprecated Use get100Messages instead
	 * @param channel The channel UUID
	 ...
	 * @returns An array of messages
	 */
	async getMessagesFrom (channel: string, uuid: string): Promise<Message[]> {
		if (this.closed) throw new Error('DB closed')
